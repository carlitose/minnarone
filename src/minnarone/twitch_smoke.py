...
def _parse_args(argv: Sequence[str], *, prog: str) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--quality",
        default="audio_only",
        help="qualità Streamlink da usare per l'audio",
    )
    parser.add_argument(
        "--audio-chunk-seconds",
        type=float,
...
def _smoke_failures(
    stats: SmokeStats, *, enable_chat: bool, enable_audio: bool
) -> list[str]:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not enable_chat and not enable_audio:
        print("abilita almeno chat o audio", file=sys.stderr)
        return 2
