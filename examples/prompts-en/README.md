...
wins; otherwise the packaged Italian default is used. This set only overrides
`rules.md` and `intro.md`, so `situations.md`, `format.md`, the summarizer and
the other per-style prompts still fall back to the packaged Italian defaults —
you would see a **mixed-language** prompt.
...
`{{reason}}`) and the control tokens (`#end_conv`, `#nothing`, `RE:`, `MSG:`)
intact. The loader is fail-fast: a missing placeholder, control token or
required section aborts startup rather than degrading silently.
...
persona and language but can never weaken the security boundary.
